...
config = {
    ...
    #     HeavisideRegression,
    # ],
    "methods": [HeavisideRegression],
    "tasks": [WineQuality, Housing],
    "repeats": 2,
    "outputs": [
        ScreenOutput(),
        CSVOutput(outpath, prjname),
        H5Output(outpath, prjname),
    ],
}
...
if __name__ == "__main__":
    ...
    for task_i in range(len(config["tasks"])):
        ...
        if not res:
            print("Fail to load " + config["tasks"][task_i].name)
            continue
        ...
        for method_i in range(len(config["methods"])):
            ...
            for ri in range(config["repeats"]):
                ...
                m.fit(train)
                print("")
                print(m.model.nminusd)
                print(m.model._log_marginal_likelihood)
                print("")
