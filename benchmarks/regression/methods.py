...
class RegressionMethod(object):
    ...
    def _preprocess(self, data, train):
        ...
        if train:
            ...
            return (
                (inputs - self.data_mean) / self.data_std,
                (labels - self.labels_mean) / self.labels_std,
            )
        else:
...
class TPRegression(RegressionMethod):
    name = "TPRegression"

    def _fit(self, train_data):
        inputs, labels = train_data
        self.model = GPy.models.TPRegression(
            inputs,
            labels,
            kernel=GPy.kern.RBF(inputs.shape[-1], ARD=True)
            + GPy.kern.Linear(inputs.shape[1], ARD=True),
...
class BimodalRegression(RegressionMethod):
    name = "BimodalRegression"

    def _fit(self, train_data):
        inputs, labels = train_data
        self.model = GPy.models.BimodalRegression(
            inputs,
            labels,
            kernel=GPy.kern.RBF(inputs.shape[-1], ARD=True)
            + GPy.kern.Linear(inputs.shape[1], ARD=True),
...
class HeavisideRegression(RegressionMethod):
    name = "HeavisideRegression"

    def _fit(self, train_data):
        inputs, labels = train_data
        self.model = GPy.models.HeavisideRegression(
            inputs,
            labels,
            kernel=GPy.kern.RBF(inputs.shape[-1], ARD=True)
            + GPy.kern.Linear(inputs.shape[1], ARD=True),
...
class SVIGP_RBF(RegressionMethod):
    ...
    def _fit(self, train_data):
        ...
        k = (
            ...
            + GPy.kern.Linear(X.shape[1], ARD=True)
        ...
        [
            self.model.optimize("scg", max_iters=40, gtol=0, messages=0, xtol=0, ftol=0)
            for i in range(10)
        ]
